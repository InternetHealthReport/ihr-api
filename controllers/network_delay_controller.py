...
class NetworkDelayController:
    ...
    async def get_network_delay_alarms(
        ...
        deviation_gte: Optional[float] = Query(
            None,
            description="Significance of the AS Hegemony change."
        ),
        deviation_lte: Optional[float] = Query(
            None,
            description="Significance of the AS Hegemony change."
        ),
        page: Optional[int] = Query(
        ...
        <ul>
        <li><b>Required parameters:</b> timebin or a range of timebins (using the two parameters timebin_lte and timebin_gte).</li>
        <li><b>Limitations:</b> At most 7 days of data can be fetched per request. For bulk downloads see: <a href="https://ihr-archive.iijlab.net/" target="_blank">https://ihr-archive.iijlab.net/</a>.</li>
        </ul>
        """
        timebin_gte, timebin_lte = validate_timebin_params(
